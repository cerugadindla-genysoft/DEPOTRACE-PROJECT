...
$stmt->bind_param(
    ...
    if ($stmt->execute()) {
        header('Location: http://localhost/php/depotrace/superadmin.php?page=createuser&status=success');
        exit;
    }
    header('Location: http://localhost/php/depotrace/superadmin.php?page=createuser&status=insert_failed&msg=' . urlencode($stmt->error));
    exit;
